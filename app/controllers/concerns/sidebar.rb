	...
	def carga_sidebar(nombre, param_id)
	    ...
	    unless ['new', 'edit', 'create', 'show', 'update'].include?(action_name)
		    ...
		    if @elemento.despliegue == 'show'
				...
				set_tabla(@elemento.controlador, @elemento.controlador.classify.constantize.ordered, (not (@controlador.classify.constantize.all.count < 26 or @elemento.despliegue == 'ulist')))
		    end
		end
	end
